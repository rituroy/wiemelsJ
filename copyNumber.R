...
baseDir=paste(dirClin,"idat/",sep="")
fileList=dir(baseDir,recursive=T)
j=match(fileList[grep("Grn",fileList)],paste(phen$Beadchip,"/",phen$Beadchip,"_",phen$Position,"_Grn.idat",sep=""))
table(is.na(j))
tbl=phen[j,c("id","Beadchip","Position")]
names(tbl)=c("Sample_Name","Sentrix_ID","Sentrix_Position")
tbl$Sample_Plate=tbl$Sample_Group=tbl$Pool_ID=tbl$Project=NA
tbl$Sample_Well=phen$Well[j]
tbl$Basename=paste("/",phen$Beadchip,"/",phen$Beadchip,"_",phen$Position,sep="")[j]
tbl1=read.table(paste(system.file("extdata",package="ChAMPdata"),"/lung_test_set.csv",sep=""), sep=",", h=F, quote="", comment.char="",as.is=T,fill=T)
write.table(tbl1[1:7,1],file=paste(baseDir,"sampleInfo.csv",sep=""), sep=",", col.names=F, row.names=F, quote=F)
write.table(tbl,file=paste(baseDir,"sampleInfo.csv",sep=""), sep=",", col.names=T, row.names=F, quote=F,append=T)
...
tbl1=read.table(paste(system.file("extdata",package="ChAMPdata"),"/lung_test_set.csv",sep=""), sep=",", h=T, quote="", comment.char="",as.is=T,fill=T,skip=7)
tbl2=read.table(paste(baseDir,"I226_BeadChip_Sample_Layout_samples.txt",sep=""), sep="\t", h=T, quote="", comment.char="",as.is=T,fill=T)


baseDir=paste(dirClin,"idat/9702496005",sep="")
baseDir=paste(dirClin,"idat/",sep="")

### run champ functions separately.
myLoad <- champ.load(baseDir)
